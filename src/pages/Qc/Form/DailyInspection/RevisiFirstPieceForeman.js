...
const RevisiFirstPieceForeman = ({route, navigation}) => {
	...
	const updateActionForemanFunc = () => {
		...
		if(frData != null){
			if(updateFR != null){
				data.push(
					<View key="askjdnui2" style={{borderWidth: 0.5, borderRadius: 5, height: 40, justifyContent: 'center', paddingLeft: 5, backgroundColor: '#b8b8b8'}}>
						<Text>{updateFR}</Text>
					</View>
				)
			}else{
				data.push(
					<View key="askjdnui2" style={{borderWidth: 0.5, borderRadius: 5, height: 40, justifyContent: 'center', paddingLeft: 5}}>
	...
	const updateDecisionFunc = () => {
		...
		if(frData != null){
			if(updateFR != "OK" && updateFR != "NG"){
				data.push(
					<View key="asoidjn2jk" style={{borderWidth: 0.5, borderRadius: 5, height: 40, justifyContent: 'center', paddingLeft: 5}}>
						<Picker 
						mode="dropdown"
